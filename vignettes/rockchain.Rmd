...
knitr::opts_chunk$set(
  ...
  message = FALSE, warning = FALSE, error = FALSE, tidy = TRUE, eval = FALSE
)
```
...
```{r cap4}
library(dplyr)
cap(crypto = "ETH", convert = c("EUR", "GBP")) %>% select(15:20)

cap(crypto = "BTC", convert = "ETH") %>% select(15:17)
```
...
If you have trouble connecting, `wallet` will retry for you, up to `max_attempts` times. The default is ten attempts. It is not unusual to be rejected by the API if you make many calls. Notifications are printed to the console if `wallet` has to try more than once as well as if if fails `max_attempts` times.
